...
#define ADI_UART4_STATUS_THRE BIT(5)
#define ADI_UART4_STATUS_DR BIT(0)

#define ADI_UART4_RBR 0x20
#define ADI_UART4_THR 0x24

register_phys_mem_pgdir(MEM_AREA_IO_NSEC, ADSP_SC598_UART0_BASE,
			ADSP_SC598_UART_SIZE);
register_phys_mem_pgdir(MEM_AREA_IO_SEC, ADSP_SC598_GICD_BASE,
			ADSP_SC598_GIC_SIZE);
...
static void adsp_serial_putc(struct serial_chip *chip __unused, int ch)
{
	...
	while (!(io_read32(uart_base + ADI_UART4_STATUS) &
		 ADI_UART4_STATUS_THRE))
		;
